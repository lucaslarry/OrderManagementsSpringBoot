...
public class CategoryController implements CategoryControllerDoc {
    ...
    @PostMapping
    public ResponseEntity<CategoryDTO> insert(@RequestBody @Valid CategoryCreateDTO categoryCreateDTO) throws BancoDeDadosException, RegraDeNegocioException {
        CategoryDTO entity = service.insert(categoryCreateDTO);
        return new ResponseEntity<>(entity, HttpStatus.CREATED);
    }
    
    @PutMapping(value = "/{id}")
    public ResponseEntity<CategoryDTO> update(@PathVariable Long id, @RequestBody @Valid CategoryDTO categoryDTO) throws RegraDeNegocioException, BancoDeDadosException {
